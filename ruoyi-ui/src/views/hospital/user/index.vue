  ...
  <div class="app-container">
    ...
    <el-divider content-position="left" v-if="queryParams1.patientIdcard != null">空床信息</el-divider>
        ...
        <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="医生名称" align="center" key="nickName" prop="nickName" v-if="columns[2].visible" :show-overflow-tooltip="true" />
          <el-table-column label="部门" align="center" key="deptName" prop="dpName" v-if="columns[3].visible" :show-overflow-tooltip="true" />
          <el-table-column label="手机号码" align="center" key="phonenumber" prop="phonenumber" v-if="columns[4].visible" width="120" />
          <el-table-column label="选择" align="center" prop="userId" v-if="queryParams1.patientIdcard != null">
            <template slot-scope="scope">
              <el-button  @click="fpys(scope.row.userId)"  type="success" plain slot="reference">选择医生:{{ scope.row.nickName }}</el-button>
        </template>
          </el-table-column>
...
<script>
import { listUser, getUser, delUser, addUser, updateUser, deptTreeSelect ,listArchives,updateSplitbed} from "@/api/hospital/user";
...
export default {
  name: "User",
  dicts: ['sys_normal_disable', 'sys_user_sex','room_dept'],
  ...
  created() {
    this.queryParams.deptId = this.$route.params && this.$route.params.deptId;
    this.queryParams1.deptId = this.$route.params && this.$route.params.deptId;
    this.queryParams1.patientIdcard = this.$route.params && this.$route.params.patientIdcard;
    this.queryParams1.splitbedId = this.$route.params && this.$route.params.splitbedId;
    this.getList();
    this.getList1();
    this.getDeptTree();
  ...
  methods: {
    ...
    // },
        /**分配医生 */
      fpys(doctorId){
        this.$modal.confirm('是否为患者"' +this.queryParams1.patientName+ '"分配医生').then(response => {
          const userrow = {"splitbedId" :this.queryParams1.splitbedId,"doctorId" : doctorId,"patientId": this.queryParams1.patientId}
          return updateSplitbed(userrow)
        }).then(()=>{

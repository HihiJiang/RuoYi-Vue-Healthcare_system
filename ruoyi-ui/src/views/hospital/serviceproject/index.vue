    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['hospital:serviceproject:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['hospital:serviceproject:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['hospital:serviceproject:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['hospital:serviceproject:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="serviceprojectList" @selection-change="handleSelectionChange">
...
<script>
import { listServiceproject, getServiceproject, delServiceproject, addServiceproject, updateServiceproject } from "@/api/hospital/serviceproject";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listServiceproject(this.queryParams).then(response => {
        this.serviceprojectList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getServiceproject(serviceprojectId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.serviceprojectId != null) {
            updateServiceproject(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addServiceproject(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除医疗服务项目编号为"' + serviceprojectIds + '"的数据项？').then(function() {
        return delServiceproject(serviceprojectIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('hospital/serviceproject/export', {
        ...this.queryParams
      }, `serviceproject_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
